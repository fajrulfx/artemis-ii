#!/usr/bin/env python3
"""
Interactive 3D trajectory visualization with acceleration coloring for Artemis II mission.

Parses JPL Horizons ephemeris data and creates an interactive matplotlib plot
showing the spacecraft trajectory colored by acceleration magnitude.
"""
...
class EphemerisData:
    ...
    def acceleration_magnitude(self) -> np.ndarray:
        """Calculate acceleration magnitude (numerical derivative of velocity)."""
        dt = np.array([(self.timestamps[i+1] - self.timestamps[i]).total_seconds() 
                       for i in range(len(self.timestamps) - 1)])
        
        dvx = np.diff(self.vx) / dt
        dvy = np.diff(self.vy) / dt
        dvz = np.diff(self.vz) / dt
        
        accel = np.sqrt(dvx**2 + dvy**2 + dvz**2)
        # Pad to match original array length
        return np.concatenate([[accel[0]], accel])
...
def plot_trajectory_acceleration(artemis: EphemerisData, moon: EphemerisData):
    """Create an interactive 3D plot with acceleration-colored trajectory."""
    ...
    accel = artemis.acceleration_magnitude
    # Convert from km/s² to m/s² for more readable numbers
    accel_ms2 = accel * 1000
    
    norm = Normalize(vmin=accel_ms2.min(), vmax=accel_ms2.max())
    cmap = plt.cm.inferno
    
    # Pre-compute colors for each segment
    colors = cmap(norm(accel_ms2[:-1]))
    ...
    ax.set_ylabel('Y (km)')
    ax.set_zlabel('Z (km)')
    ax.set_title('Artemis II Trajectory - Acceleration Colored\n(Earth-Centered Ecliptic J2000)')
...
def main():
    ...
    accel = artemis.acceleration_magnitude * 1000  # m/s²
    print(f"Loaded {len(artemis.timestamps)} data points")
    print(f"Acceleration range: {accel.min():.4f} - {accel.max():.4f} m/s²")
    
    plot_trajectory_acceleration(artemis, moon)
    plt.show()
